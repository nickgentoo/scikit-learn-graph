...
if __name__=='__main__':
    ...
    if len(sys.argv)<1:
        sys.exit("python ODDKernel_example.py dataset r l filename kernel C")
    dataset=sys.argv[1]
    ...
    #WCMS=np.zeros(shape=(features.shape[1],1))
    WCMS=csr_matrix((features.shape[1],1))
    ...
    for i in xrange(features.shape[0]):
          ...
          #dot=0.0
          module=(exCMS.T*exCMS)[0,0]
          #print "module", module

          time2=time.time()
          cms_creation+=time2 - time1
          dot= (WCMS.T*exCMS)[0,0]
          #print "dot", dot
