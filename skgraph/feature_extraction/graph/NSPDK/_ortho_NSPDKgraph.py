...
class OrthogonalVectorizer(object):
    """
    ...
    def _transform_serial(self, G_list):
        feature_matrices = []
        feature_dicts = defaultdict(lambda: defaultdict(float))
        for instance_id , G in enumerate( G_list ):
            tmpfeats = self._transform( instance_id, G )
            for key, phi in tmpfeats.items():
                if key in feature_dicts.keys():
                    feature_dicts[key].update(phi)
                else:
                    feature_dicts[key] = phi

        for key, phi in feature_dicts.items():
            feature_matrices.append(self._convert_to_sparse_matrix(phi))
...
#                    key = ",".join([str((radius-2) / 2), str((distance - 2) / 2)])
    ...
    def _normalization(self, feature_list, instance_id):
        ...
        feature_vector = defaultdict(lambda : defaultdict(float))
        total_norm = 0.0
        for key, features in feature_list.items():
            norm = 0
            ...
            for feature, count in features.items():
                ...
                feature_vector[key][feature_vector_key] = feature_vector_value
                total_norm += feature_vector_value*feature_vector_value
        #global normalization
        if self.normalization:
            normalizationd_feature_vector = defaultdict(lambda : defaultdict(float))
            sqrt_total_norm = math.sqrt( float(total_norm) )
            for key, feature_items in feature_vector.items():
                for feat, value in feature_items.items():
                    normalizationd_feature_vector[key][feat] = value/sqrt_total_norm
...
class Annotator(OrthogonalVectorizer):
    def __init__(self,
        estimator = SGDClassifier(),
        vectorizer = OrthogonalVectorizer(),
        reweight = 1.0,
