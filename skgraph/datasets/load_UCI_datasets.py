...
def dispatch(dataset):
    if dataset=="a9a":
        print "Loading a9a(training) dataset"
        tr,trt=load_a9a()
        te,tet=load_a9at()
        return Bunch(training=tr.tocsr(),
                     training_target=trt,
                     test=te,
                     test_target=tet)
    elif dataset=="a9atest":
        print "Loading a9a(test) dataset"
        g_it=load_a9at()
    elif dataset=="CPDB":
...
def load_a9a():
    ...
    data : Bunch
        ...
        sample.
    """
    input_data_url='../Data/a9a.libsvm'
    #f = requests.get(input_data_url) #.text.split('\n')

    data,target=load_svmlight_file(input_data_url)
    return data,target
...
def load_a9at():
    ...
    data : Bunch
        ...
        sample.
    """
    input_data_url='../Data/a9a.t.libsvm'
    #f = requests.get(input_data_url) #.text.split('\n')
